...
import { opWorkJobTagControllerDeleteOne, opWorkJobTagControllerFindMany, opWorkJobTagControllerUpdateOne } from '#/generated/client';
import type { OpWorkJobTag } from '#/generated/prisma/browser';
import { $t } from '#/locales';
import { useOpWorkJobTagColumns, useOpWorkJobTagFilterFormSchema } from './OpWorkJobTagData';
...
const [Grid, gridApi] = useVbenVxeGrid({
  formOptions: {
    schema: useOpWorkJobTagFilterFormSchema(),
    ...
    proxyConfig: {
      ajax: {
        query: async (options: {
          ...
          return await opWorkJobTagControllerFindMany({
            query: {
...
function onDelete(row: OpWorkJobTag) {
  ...
  opWorkJobTagControllerDeleteOne({ path: { id: row.id } })
    .then((data) => {
      if (data.error) {
