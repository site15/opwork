<script lang="ts" setup>
import { computed, nextTick, ref } from 'vue';
import { notification } from 'ant-design-vue';

import { useVbenDrawer } from '@vben/common-ui';

import { useVbenForm } from '#/adapter/form';
import { opWorkProfileControllerCreateOne, opWorkProfileControllerUpdateOne } from '#/generated/client';
import type { OpWorkProfile } from '#/generated/prisma/browser';
import { $t } from '#/locales';
import { useOpWorkProfileFormSchema } from './OpWorkProfileData';
import { applyBackendValidationErrors } from '#/utils/apply-backend-validation-errors';

const emits = defineEmits(['success']);

const formData = ref<OpWorkProfile>({} as OpWorkProfile);

const [Form, formApi] = useVbenForm({
  schema: useOpWorkProfileFormSchema(),
  showDefaultActions: false,
});

const id = ref();
const [Drawer, drawerApi] = useVbenDrawer({
  async onConfirm() {
    /// const { valid } = 
    await formApi.validate();
    // if (!valid) return;
    const values = await formApi.getValues();
    drawerApi.lock();
    (id.value ? opWorkProfileControllerUpdateOne({
      path: { id: id.value },
      body: {
        type: values.type,
        userType: values.userType,
        email: values.email,
        phone: values.phone,
        website: values.website,
        location: values.location,
        avatarUrl: values.avatarUrl,
        coverImage: values.coverImage,
        AuthUser: { connect: { id: values.userId } },
      }
    }) : opWorkProfileControllerCreateOne({
      body: {
        type: values.type,
        userType: values.userType,
        email: values.email,
        phone: values.phone,
        website: values.website,
        location: values.location,
        avatarUrl: values.avatarUrl,
        coverImage: values.coverImage,
        AuthUser: { connect: { id: values.userId } },
      }
    }))
      .then(() => {
        emits('success');
        drawerApi.close();
      })
      .catch((err) => {
        drawerApi.unlock();
        const hasValidationErrors = applyBackendValidationErrors(formApi, err);
        if (!hasValidationErrors) {
          notification.error({
            message: id.value ? $t('actions.common.updateFailed') : $t('actions.common.createFailed'),
            description: err instanceof Error ? err.message : '',
            duration: 3000,
          });
        }
      });
  },

  async onOpenChange(isOpen) {
    if (isOpen) {
      const data = drawerApi.getData<OpWorkProfile>();
      formApi.resetForm();

      if (data) {
        formData.value = data;
        id.value = data.id;
      } else {
        id.value = undefined;
      }

      // Wait for Vue to flush DOM updates (form fields mounted)
      await nextTick();
      if (data) {
        formApi.setValues(data);
      }
    }
  },
});

const getDrawerTitle = computed(() => {
  return formData.value?.id
    ? $t('common.edit', $t('resource.name.OpWorkProfile'))
    : $t('common.create', $t('resource.name.OpWorkProfile'));
});

</script>
<template>
  <Drawer :title="getDrawerTitle">
    <Form>
    </Form>
  </Drawer>
</template>
<style lang="css" scoped>
:deep(.ant-tree-title) {
  .tree-actions {
    display: none;
    margin-left: 20px;
  }
}

:deep(.ant-tree-title:hover) {
  .tree-actions {
    display: flex;
    flex: auto;
    justify-content: flex-end;
    margin-left: 20px;
  }
}
</style>
