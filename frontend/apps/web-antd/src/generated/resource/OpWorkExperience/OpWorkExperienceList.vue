...
import { opWorkExperienceControllerDeleteOne, opWorkExperienceControllerFindMany, opWorkExperienceControllerUpdateOne } from '#/generated/client';
import type { OpWorkExperience } from '#/generated/prisma/browser';
import { $t } from '#/locales';
import { useOpWorkExperienceColumns, useOpWorkExperienceFilterFormSchema } from './OpWorkExperienceData';
        ...
        query: async (options: {
          ...
          return await opWorkExperienceControllerFindMany({
            query: {
              ...
              sort: (options.sort?.field && options.sort?.order) ? `${options.sort.field}:${options.sort.order}` : 'company:desc'
            },
          }).then(async (result) => {
            if (result?.error) {
