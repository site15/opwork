  ...
  gridOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async (options: {
          ...
          return await opWorkApplicationControllerFindMany({
            query: {
              ...
              sort: (options.sort?.field && options.sort?.order) ? `${options.sort.field}:${options.sort.order}` : 'coverLetter:desc'
            },
          }).then(async (result) => {
            if (result?.error) {
        ...
        statusNotes: item.statusNotes||undefined,
        appliedAt:item.appliedAt?dayjs(item.appliedAt):undefined,
        statusUpdatedAt:item.statusUpdatedAt?dayjs(item.statusUpdatedAt):undefined,
              })),
    ...
    sortConfig: {
      defaultSort: { field: 'coverLetter', order: 'desc' },
      remote: true,
    },
    rowConfig: {
      keyField: 'id',
