...
На экране авторизации интерфейс визуально разделен на две части: слева промо-блок с иллюстрацией, заголовком и описанием, справа форма входа.
В правом верхнем углу страницы есть быстрые настройки интерфейса (иконки темы/языка/режима), ниже в форме есть ссылка "Создать аккаунт".
туду:
...
Форма регистрации находится справа и содержит: email, пароль, подтверждение пароля, выбор типа профиля, чекбокс согласия и кнопку "Регистрация".
Под формой есть быстрый переход назад к авторизации.
туду:
...
На странице видно левую панель фильтров и правую рабочую область со списком вакансий.
В шапке списка доступны селекты сортировки (по зарплате, дате публикации, откликам), а также кнопка добавления вакансии.
Для пустого списка отображается явное состояние "Нет результатов".
...
Карточка вакансии в списке показывает ключевые данные: название, краткие параметры и действия для перехода к детальному просмотру/редактированию.
...
Каждая заявка в списке отображается карточкой: имя/позиция кандидата, статус, ожидаемая зарплата, дата подачи и краткое сопроводительное письмо.
Слева доступна компактная панель фильтров (поиск по тексту и статус), сверху блока списка доступны дополнительные сортировки.
...
Страница детального просмотра визуально разделена на основной блок и правую панель.
Справа расположен блок управления статусом (селект статуса, поле примечания и кнопка сохранения), ниже — предпочтения и контакты соискателя.
В основном блоке видно карточки "Соискатель", "Вакансия", сопроводительное письмо и сводку по резюме.
...
Форма резюме содержит персональные поля (ФИО, дата рождения, пол), профессиональные поля (текущая должность/компания, краткое резюме, зарплата/валюта) и переключатели готовности (к работе, удаленке, переезду).
Ниже основной формы расположены табличные блоки (образование, опыт, навыки) с кнопкой создания записей.
...
Поле глобального поиска расположено в верхней панели и доступно из рабочих разделов, чтобы быстро переходить к найденным сущностям.
...
Нотификации открываются выпадающей панелью из верхнего бара: в каждой записи видны заголовок, текст, дата/время и кнопки действий (прочитано/архив).
Внизу панели есть кнопки массовых действий, включая очистку и переход к полному списку уведомлений.
туду:
